...
object tcp {
  ...
  /**
   * TCP based transport. This use a fixed size Int to encode the length of a message.
   */
  def tcpTransport(maxConnections: Int,
                   connectionTimeout: Duration,
                   sendTimeout: Duration): ZIO[Clock, Nothing, Transport] =
      ...
      new Transport {
        val transport: Transport.Service[Any] = new Transport.Service[Any] {
          ...
          override def send(to: SocketAddress, data: Chunk[Byte]) =
            AsynchronousSocketChannel().use { client =>
              val size = data.size
              for {
                _ <- client.connect(to)
          ...
          override def bind(addr: SocketAddress) =
            ZStream.unwrapManaged {
              AsynchronousServerSocketChannel()
                ...
                .withEarlyRelease.map { case (close, server) =>
                      ...
                      ZStream.unwrapManaged {
                        connection.map { channel =>
                          ZStream
                            .repeatEffect(
                              for {
                                length <- channel.read(4).flatMap(c => ZIO.effect(new BigInteger(c.toArray).intValue()))
