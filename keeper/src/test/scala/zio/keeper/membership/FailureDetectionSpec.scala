      ...
      Nodes.make.flatMap(
        nodes =>
          for {
            ...
            ping <- protocol.produceMessages
                     .take(2)
                     .collect { case Message.WithTimeout(message, _, _) => message }
                     .runCollect
          } yield assert(ping)(
            equalTo(List(Message.Direct(nodeAddress2, Ping(1)), Message.Direct(nodeAddress1, Ping(2))))
          )
      )
    }
